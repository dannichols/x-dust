...
public class XDust {
	...
	public XDustNode compile(String str, String name, String sourceFile) {
		...
		this.getTemplates().put(name, new XDustTemplate(name, rootNode, sourceFile));
		return rootNode;
	}
	...
	public XDustNode load(String sourceFile, String name) throws IOException {
		...
		if (this.getTemplates().containsKey(name)) {
			...
			if (sourceFile.startsWith("~")) {
				sourceFile = sourceFile.replace("~", this.getContextPath() == null ? "" : this.getContextPath()).replace("//", "/");
			}
